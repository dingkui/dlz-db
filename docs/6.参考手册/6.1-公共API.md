...
本页汇总 DLZ-DB 8.0 面向业务代码的公开入口。它是使用索引，不替代源码签名，也不罗列实现包中的类型。AI 生成代码时应优先读取 [DLZ-DB AI 编程速读](../5.AI辅助/dlz-db-速读.md)。
...
- 实体注解和单次操作选项：[注解与选项](./6.3-注解与选项.md)
- 稳定业务 API、扩展 SPI 与实现包的承诺范围：[API、SPI 与实现边界](./6.4-API-SPI与实现边界.md)
- Java、框架、事务和 SQL 等现实限制：[兼容性与限制](./6.5-兼容性与限制.md)
