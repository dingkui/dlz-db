...
// 切换到报表库
List<ResultMap> report = DB.ds.use("report", () ->
    DB.jdbc.selectWrapper("SELECT * FROM daily_report").queryList()
);
```
...
// 如需手动替换：
DB.ds.setDefaultDataSource(yourDataSource);
```

## 缓存说明

当前代码中的缓存主要用于实体元数据和表结构信息缓存。仓库内没有可确认的 Solon `JedisPool` 自动装配或 `ICacheExecutor` 实现，因此本插件文档不再承诺 Redis 自动集成；如需业务缓存，请使用项目自己的缓存组件。
