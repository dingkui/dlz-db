...
// 主库写入
DB.Dynamic.use("master", () -> DB.Pojo.insert(user).execute());
```
...
public class UserService {
    ...
    public void createUser(User user) {
        ...
        DB.Pojo.insert(userLog).execute();
    }
}
```
...
public class OrderService {
    ...
    public void createOrder(Order order) {
        // 主库写订单
        DB.Dynamic.use("master", () -> DB.Pojo.insert(order).execute());
        
        // 报表库写统计
        DB.Dynamic.use("report", () -> DB.Pojo.insert(stats).execute());
    }
}
```
