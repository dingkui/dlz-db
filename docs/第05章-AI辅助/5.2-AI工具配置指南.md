...
- `queryBean()` → 严格单个 Bean，多条抛异常
- `queryFirstBean()` → 非严格取第一条 Bean
- `queryBeanList()` → List<Bean>
- `queryBeanPage()` → Page<Bean>
- `queryOne()` → 严格单条 ResultMap，多条抛异常
- `queryFirst()` → 非严格取第一条 ResultMap
- `queryList()` → List<ResultMap>
- `queryOne(Class)` → 严格单条指定类型
- `queryFirst(Class)` → 非严格取第一条指定类型
- update/delete Wrapper 必须以 `.execute()` 结尾；直接 CRUD 会立即执行

### 硬约束

1. 无需自写 Mapper/DAO，直接使用 `DB.pojo.*` 和框架 Wrapper
2. 占位符不混用：`DB.jdbc` 用 `?`；`DB.sql` 用 `#{key}`
3. 查询列用 `select()`
4. LIKE 用 `like()` / `likeLeft()` / `likeRight()` / `notLike()`
5. Pojo 插入用 `DB.pojo.insert(entity)`，自增主键回填实体；Table 插入可用 `insertWithAutoKey()` 取得 `Long`
6. 物理删除优先使用 `DeleteOption.PHYSICAL` 或 PojoDelete `.physical()`；`SelectOption.INCLUDE_DELETED` 用于支持 `DbOption...` 的直接查询（如 `selectById`），当前 Pojo 查询 Wrapper 没有对称的稳定便捷方法
7. `in()` 不可传标量单值，必须传数组/Collection、CSV 字符串或 "sql:子查询"
...
// 插入
DB.pojo.insert(user); // 执行插入并将生成的主键回填到实体
...
- 写操作必须 `.execute()` 结尾
- `in()` 不能传标量单值，应传数组/Collection、CSV 或 `sql:` 子查询
- 查询列用 `select()`
````
...
- update/delete Wrapper：必须 .execute() 结尾；直接 CRUD 立即执行
...
```json
{
  "ai": {
    ...
    "codeTemplates": {
      "query": "DB.pojo.selectWrapper({Entity}.class).eq({Entity}::{Field}, {Value}).queryBean()",
      "queryList": "DB.pojo.selectWrapper({Entity}.class).queryBeanList()",
      "insert": "DB.pojo.insert({entity})",
      "update": "DB.pojo.updateWrapper({Entity}.class).set({Entity}::{Field}, {Value}).eq({Entity}::getId, {id}).execute()",
      "delete": "DB.pojo.deleteWrapper({Entity}.class).eq({Entity}::getId, {id}).execute()"
    }
  }
}
```
...
## 硬约束
1. 无需自写 Mapper/DAO，直接 DB.pojo.*
2. 占位符不混用
3. 查询列用 select()
4. in() 不能传标量单值，应传数组/Collection、CSV 或 sql: 子查询
...
public Page<{Entity}> page{Entity}(Integer pageNum, Integer pageSize, {Params}) {
    return DB.pojo.selectWrapper({Entity}.class)
