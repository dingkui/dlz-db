...
// → UPDATE user SET status = 0 WHERE deleted = 0
```
...
```java
DB.pojo.deleteWrapper(User.class).eq(User::getId, 1).ignoreLogicDelete(true).execute();
// 执行真正的 DELETE
```
...
// → DELETE FROM user WHERE deleted = 0（不会删全表）
```
