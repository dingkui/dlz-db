...
public class User {
    ...
    private String name;
    private Integer age;
    private Integer deleted;       // 存在即启用逻辑删除
...
| `@TableId` | 指定主键和生成策略 | `@TableId(type = IdType.AUTO)` |

> 框架可以将名为 `id` 的字段识别为主键，但不会因此推断数据库自增策略。需要生成或回填 ID 时应显式标注 `@TableId`；其默认策略是 `IdType.SEQ`。
...
| 方法 | 返回 | 说明 |
|------|------|------|
| `queryBean()` | `T` / null | 严格单条 Bean；多条抛出异常 |
| `queryFirstBean()` | `T` / null | 非严格取第一条 Bean |
| `queryOne()` | `ResultMap` / null | 严格单条 Map；多条抛出异常 |
...
| `queryOne(Class)` | 指定类型 / null | 严格单条指定类型对象；多条抛出异常 |
| `queryFirst(Class)` | 指定类型 / null | 非严格取第一条指定类型对象 |
