...
| `ni(字段, 值列表)` | 不在列表中 | `column NOT IN (...)` |
| `lk(字段, 值)` | 模糊匹配 | `column LIKE '%值%'` |
| `ll(字段, 值)` | 左模糊 | `column LIKE '值%'` |
| `lr(字段, 值)` | 右模糊 | `column LIKE '%值'` |
| `nl(字段, 值)` | 非模糊 | `column NOT LIKE '%值%'` |
| `bt(字段, 值1, 值2)` | 范围 | `column BETWEEN ? AND ?` |
| `nb(字段, 值1, 值2)` | 非范围 | `column NOT BETWEEN ? AND ?` |
...
// NOT IN
.ni(User::getName, "admin,root")

// BETWEEN
.bt(User::getAge, 18, 30)
.bt(User::getAge, "18,30")

// NOT BETWEEN
.nb(User::getScore, 0, 60)
```
...
```java
DB.Pojo.select(User.class)
    .lk(User::getName, "张")          // LIKE '%张%'
    .ll(User::getPhone, "138")        // LIKE '138%'
    .lr(User::getAddress, "北京")      // LIKE '%北京'
    .nl(User::getDescription, "测试")  // NOT LIKE '%测试%'
    .queryBeanList();
```
