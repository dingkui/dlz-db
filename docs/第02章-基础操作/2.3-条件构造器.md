...
// 复用到不同操作
DB.table.selectWrapper("user").where(condition).queryList();
DB.table.updateWrapper("user").set("flag", 1).where(condition).execute();
DB.table.deleteWrapper("user").where(condition).execute();
```
