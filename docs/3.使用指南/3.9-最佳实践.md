...
- `updateWrapper/deleteWrapper` 等 Wrapper 写操作必须以 `.execute()` 结束；直接 CRUD 和 `DB.batch` 调用时已经执行
- `in/notIn` 优先传 `Collection`、`Object[]`、CSV 字符串或 `"sql:子查询"`；`Number` 标量虽然可用，但单值应写 `eq/ne`，不要传 `long[]`、`int[]` 等原生类型数组
