...
| `likeLeft(字段, 值)`      | 后缀匹配 | `column LIKE '%值'` |
| `likeRight(字段, 值)`      | 前缀匹配 | `column LIKE '值%'` |
...
DB.pojo.selectWrapper(User.class)
    .eq(User::getStatus, 1)
    .ors(w -> w.eq(User::getName, "张三").gt(User::getAge, 20))
    .queryBeanList();

// (type=1 OR type=2) AND (status=1 AND level>3)
DB.pojo.selectWrapper(User.class)
    .ors(w -> w.eq(User::getType, 1).eq(User::getType, 2))
    .ands(w -> w.eq(User::getStatus, 1).gt(User::getLevel, 3))
    .queryBeanList();
```
...
8.0 的稳定业务 API 是在各类 Wrapper 上直接构建条件。`Condition` 位于 `com.dlz.db.internal` 实现包，不应在业务代码中直接导入或作为自定义 API 的参数类型。需要复用时，将条件组装封装成面向具体 Wrapper 的业务方法：

```java
private PojoQuery<User> activeAdults(PojoQuery<User> query) {
    return query.eq(User::getStatus, 1)
                .gt(User::getAge, 18);
}

List<User> users = activeAdults(DB.pojo.selectWrapper(User.class))
    .queryBeanList();
```
