...
- **面向接口编程**：核心抽象为 `ISqlExecutor`/`ITxExecutor` 等接口，易于扩展
...
- `DB.pojo.selectWrapper(...).eq(...).queryBean()` → 不变
- `DB.table.selectWrapper(...)` → 不变
- `DB.jdbc.selectWrapper(sql, args)` → 不变
- `DB.sql.selectWrapper("key.xxx")` → 不变
- `DB.batch.insert(...)` → 不变
- `DB.ds.use(name, () -> ...)` → 不变
- `DB.tx.run(() -> ...)` → 不变
...
+// DB.pojo.selectWrapper(...), DB.jdbc.selectWrapper(...)
+
