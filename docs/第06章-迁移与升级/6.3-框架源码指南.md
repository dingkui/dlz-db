...
- 不提供业务级一二级缓存；实体元数据和表结构使用 core 内部缓存
...
│       ├── config/
│       │   ├── SpringDlzDbConfig.java       # Spring Boot 自动配置
│       │   ├── SpringDlzDbProperties.java     # yml 配置绑定
...
└── dlz-db-solon-plugin/                   (Solon 适配)
    └── src/main/java/com/dlz/db/solon/
        ├── DlzDbSolonPlugin.java          # Solon SPI 插件入口
...
DB.ds.use("slave", () -> {
    ...
    DB.pojo.selectWrapper(User.class).queryList();
});
```
...
自动装配入口：`SpringDlzDbConfig.java`（用 `@Bean` 注册各组件）。
...
3. 在 `DialectRegistry` 中注册方言

#### 场景2：添加新的条件方法

1. 在 `inf/` 包中找到对应接口添加方法签名
2. 在 `Condition.java` 中实现链式调用
3. 在 `SchemaDialect` 中实现数据库结构和元数据逻辑
4. 编写测试用例验证
...
public void testSelect() {
    ...
    // 执行测试
    User result = DB.pojo.selectWrapper(User.class)
    ...
    // 清理数据
    DB.pojo.deleteWrapper(User.class).eq(User::getId, user.getId()).execute();
}
```
