...
Starter 使用 Spring 容器中的 `DataSource` 自动初始化。不需要额外编写 `DlzDbConfigs` 或继承 DLZ-DB 配置类。共存期间应确认 MyBatis 和 DLZ-DB 使用同一个预期 `DataSource`。

## 2. 实体注解

MyBatis-Plus 与 DLZ-DB 的注解类名相似，但包名不同，不能简单声称“无需修改”。迁移后使用：
