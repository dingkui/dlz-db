...
逻辑删除开启后查询自动带 `deleted = 0` 条件。如需查询已被逻辑删除的数据，可以使用 `ignoreLogicDelete(true)`：

```java
List<User> deletedUsers = DB.pojo.selectWrapper(User.class)
    .ignoreLogicDelete(true)
    .eq(User::getStatus, 1)
    .queryBeanList();
// → SELECT * FROM user WHERE status = 1 （无 deleted = 0 过滤）
```
...
```java
DB.pojo.deleteWrapper(User.class).ignoreLogicDelete(true).eq(User::getId, 1).execute();
