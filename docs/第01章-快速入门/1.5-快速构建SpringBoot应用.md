...
- **物理删除**：PojoDelete 使用 `.physical()`，直接删除 API 传 `DeleteOption.PHYSICAL`

> 实体或数据表存在已配置的逻辑删除字段时，框架在插入时会自动补入 `deleted = 0`，无需业务代码手动设置。
