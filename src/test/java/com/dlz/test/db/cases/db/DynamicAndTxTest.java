...
/**
 * 测试 {@link com.dlz.db.ds.DBDynamic#use} 与 {@link com.dlz.db.ds.DBTx#run} 的拆分语义：
 * <ul>
...
public class DynamicAndTxTest extends SpingDbBaseTest {
    ...
    private static final String DB_FILE = "tx_test.sqlite3";
    ...
    public void testTxRun_rollbackOnException() {
        ...
        Integer count = DB.Dynamic.use(TEST_DS_NAME, () ->
                DB.Jdbc.select("SELECT COUNT(*) FROM tx_user WHERE id = ?", 200).count()
        );
        assertEquals("异常应触发回滚，数据不应持久化", Integer.valueOf(0), count);
    }
