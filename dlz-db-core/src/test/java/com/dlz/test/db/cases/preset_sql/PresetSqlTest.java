...
public class PresetSqlTest extends BaseDBTest {
    ...
    public void sql_update() {
        ...
        assertEquals(Integer.valueOf(77),  DB.jdbc.selectWrapper("SELECT age FROM user WHERE name=?", "alice").queryInt());
    }
    ...
    public void sql_insert() {
        ...
        assertEquals(1, DB.jdbc.selectWrapper("SELECT COUNT(*) FROM user WHERE name=?", "sql_ins").count());
    }

    @Test
    public void sql_delete() {
        DB.sql.execute("DELETE FROM user WHERE name=#{name}",new JSONMap("name", "alice"));
        assertEquals(0, DB.jdbc.selectWrapper("SELECT COUNT(*) FROM user WHERE name=?", "alice").count());
    }
