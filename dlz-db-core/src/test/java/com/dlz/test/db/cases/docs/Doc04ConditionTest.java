...
public class Doc04ConditionTest extends BaseDBTest {
    ...
    public void conditionTest4_1_2() {
        //驼峰参数，自动转下划线
        final List<User> users = DB.pojo.selectWrapper(User.class)
                .isNull("deleteTime")  // delete_time IS NULL
                .queryBeanList();
        //条件sql: WHERE DELETE_TIME IS NULL

        //下划线字段
        final List<ResultMap> users2 = DB.pojo.selectWrapper(User.class)
    ...
    public void conditionTest4_5_4() {
        DB.pojo.selectWrapper(User.class).between(User::getAge, 18, 30).queryList();
        //条件sql: where age BETWEEN 18 AND 30

        DB.pojo.selectWrapper(User.class).between(User::getAge, Arrays.asList(18, 30)).queryList();
        //条件sql: where age BETWEEN 18 AND 30

        DB.pojo.selectWrapper(User.class).between(User::getAge, Arrays.asList(18, 30)).queryList();
        //条件sql: where age BETWEEN 18 AND 30

        DB.pojo.selectWrapper(User.class).between(User::getAge, new Integer[]{18, 30}).queryList();
        //条件sql: where age BETWEEN 18 AND 30
        ...
        DB.pojo.selectWrapper(User.class).between(User::getAge, new String[]{"18", "30"}).queryList();
        //条件sql: where age BETWEEN '18' AND '30'

        Date startDate = DateUtil.getDate("2020-01-01");
        Date endDate = DateUtil.getDate("2021-01-01");
        DB.pojo.selectWrapper(User.class).between(User::getCreateTime, startDate, endDate).queryList();
        //条件sql: where CREATE_TIME BETWEEN '2020-01-01 00:00:00' AND '2021-01-01 00:00:00'
    }

    @Test
    public void conditionTest4_5_5() {
        DB.pojo.selectWrapper(User.class).notBetween(User::getScore, 0, 60).queryList();
