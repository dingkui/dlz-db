...
/**
 * ICondAddByFn 接口测试 - 基于解耦Lambda的条件构造（支持跨Bean）
 */
@DisplayName("ICondAddByFn 解耦Lambda条件测试")
class ICondAddByFnTest {
    ...
    @Test
    @DisplayName("测试跨Bean - 混用不同Bean的Lambda")
    void testCrossBeanMixed() {
        // ICondAddByFn 的核心特性：可以混用不同Bean的Lambda
