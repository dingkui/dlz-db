...
import com.dlz.db.wrapper.SqlQuery;
...
public class DBSqlSelectTest extends BaseDBTest {
    ...
    public void sqlSelectTest1() {
        ...
        SqlQuery ump2 = DB.sql.selectWrapper(sql);
        ump2.addPara("a", "a1");
        ump2.addPara("b", "b1");
        ump2.addPara("d", "d1");
        ump2.addPara("c", "c1");
    ...
    public void sqlSelectTest3(){
        final SqlQuery sqlQuery = DB.sql.selectWrapper("SELECT t.* FROM PTN t WHERE t.id=${key.comm.pageSql} AND t.cc=${a} AND c=${b} AND ccc")
                .addPara("a", "a${b}")
                .addPara("b", "b${c}")
                .addPara("_sql", "_sql${a}");
