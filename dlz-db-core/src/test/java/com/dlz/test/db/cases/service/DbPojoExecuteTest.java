...
class DbPojoExecuteTest extends BaseDBTest {
    ...
    void testInsertSuccess() {
        ...
        TestAutoEntity result = dbPojo.insert(entity);
    ...
    void testInsertThenSelect() {
        TestAutoEntity entity = new TestAutoEntity();
        entity.setName("王五");
        entity.setAge(30);

        dbPojo.insert(entity);

        // 查询验证
        TestAutoEntity found = dbPojo.selectById(TestAutoEntity.class, entity.getId());
    ...
    void testUpdateById() {
        ...
        entity.setName("新名称");
        entity.setAge(26);
        TestAutoEntity result = dbPojo.updateById(entity);

        assertNotNull(result, "更新后应返回对象");

        // 验证更新结果
        TestAutoEntity updated = dbPojo.selectById(TestAutoEntity.class, entity.getId());
    ...
    void testDeleteById() {
        ...
        // 删除
        int rows = dbPojo.deleteById(TestAutoEntity.class, entity.getId());
    ...
    void testDeleteByIds() {
        ...
        // 批量删除
        int rows = dbPojo.deleteByIds(TestAutoEntity.class, ids);

        assertEquals(3, rows, "应删除3条记录");

        // 验证删除结果
        for (Long id : ids) {
            TestAutoEntity found = dbPojo.selectById(TestAutoEntity.class, id);
            assertNull(found, "删除后应查询不到数据");
        }
    }
    ...
    void testSelectByCondition() {
        ...
        // 条件查询
        PojoQuery<TestAutoEntity> wrapper = dbPojo.select(TestAutoEntity.class);
